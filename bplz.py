...
def downloadRun(bufferTime, url):
    ...
    browser = webdriver.Firefox(options=firefox_options, executable_path='geckodriver')
    browser.get(url)
    ...
    for ready in readys:
        ...
        browser1 = webdriver.Firefox(firefox_profile=profile, options=firefox_options, executable_path='geckodriver')
...
def rename(directory, file_name):
    filelist = os.listdir(directory)
...
def main(argv):
    ...
    directory = downloadRun(int(buftime), url)
    file_name = getRealZip(directory)
    print("开始修改文件名")
